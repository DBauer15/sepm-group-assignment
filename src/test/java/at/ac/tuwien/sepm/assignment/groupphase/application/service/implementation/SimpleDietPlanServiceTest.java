...
public class SimpleDietPlanServiceTest extends BaseTest {
	...
	public void testCreate_invalidDataExceedsUpperLimits_notCallsPersistenceCreateAndValidations()
		...
		try {
			...
			Assert.assertEquals("Enter a value that is smaller than 100.0 in the field 'Lipid'", errors.get(1));
			Assert.assertEquals("Enter a value that is smaller than 100.0 in the field 'Protein'", errors.get(2));
			Assert.assertEquals("Enter a value that is smaller than 100.0 in the field 'Carbohydrate'", errors.get(3));
			Assert.assertEquals("The sum of 'Lipid', 'Protein' and 'Carbohydrate' has to be smaller than or equal to 100%", errors.get(4));
			return;
		}
